...
classdef NetSolver_layers_norm < handle
    ...
    methods
        function obj = NetSolver_layers_norm(X,V,Z, hiddenLayerSize, epoch, reg)
        ...
        function solve(obj, y_x)
            ...
            norm_val = max(abs(y_x));
            net0 = trainNetwork(obj.X,y_x/norm_val ,obj.layers,obj.options);
            
            obj.yest_x = predict(net0, obj.X)*norm_val;
            obj.yest_v = predict(net0, obj.V)*norm_val;
            obj.yest_z = predict(net0, obj.Z)*norm_val;
            obj.net = net0;
        end
